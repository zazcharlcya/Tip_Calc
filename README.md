...
- **`tip_calculator_history.json`** — история расчетов в JSON.  
- **`tip_calculator_history.csv`** — создается после экспорта.  
...
- Убедитесь, что файл `tip_calculator.py` в той же папке, где вы запускаете программу.  
...
### Улучшенный `requirements.txt`  

```
# Для работы нужен только tkinter (обычно встроен в Python)
# Но если возникают ошибки, его можно установить отдельно (см. README.md)
tkinter>=0.1.0  # Минимальная версия (обычно уже есть в Python)

# Дополнительно (если нужно собирать в exe)
# pyinstaller==6.0.0
```
